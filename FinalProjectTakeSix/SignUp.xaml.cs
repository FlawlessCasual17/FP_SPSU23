...
namespace FinalProjectTakeSix {
    ...
    public partial class SignUp : ContentPage {
        ...
        public SignUp() {
            ...
            // Uncomment the line below to insert dummy data
            InsertDummyData();
        ...
        private void OnClickSignUp(object sender, EventArgs e) {
            ...
            var newMember = new Member(
                ...
                EmailEntry.Text,
                GenderEntry.Text,
                DateTime.Parse(DateOfBirthEntry.Text),
                MembershipTypeEntry.Text,
                HealthInformationEntry.Text,
                FitnessGoalsEntry.Text,
                EmergencyContactNameEntry.Text,
                EmergencyContactNumberEntry.Text,
                PreferredWorkoutTimeEntry.Text,
                int.Parse(TrainingSessionsPerWeekEntry.Text),
                bool.Parse(MedicalConditionsEntry.Text),
                bool.Parse(PersonalTrainingEntry.Text),
                bool.Parse(OptInForNewsletterEntry.Text),
                DateTime.Parse(LastCheckInDateTimeEntry.Text));

            // Call the InsertMember method through the dbService instance
        ...
        private bool ValidateInput() {
            ...
            if (!ValidateRequiredField(HealthInformationEntry, "Health Information"))
                return false;
            if (!ValidateFitnessGoals(FitnessGoalsEntry.Text)) return false;

            if (!ValidateRequiredField(EmergencyContactNameEntry, "Emergency Contact Name"))
                return false;
            if (!ValidateRequiredField(EmergencyContactNumberEntry, "Emergency Contact Number"))
                return false;
            ...
            if (!ValidateBooleanField(MedicalConditionsEntry, "Has Medical Conditions"))
                return false;
            if (!ValidateBooleanField(PersonalTrainingEntry, "Wants Personal Training"))
                return false;
            return ValidateBooleanField(OptInForNewsletterEntry, "Opted In for Newsletter")
                && ValidateDateField(LastCheckInDateTimeEntry, "Last Check-In Date Time");
        ...
        private bool ValidatePhoneNumber(string phoneNumber) {
            ...
            const string phoneNumberPattern = @"^\d{10}$";

            if (Regex.IsMatch(phoneNumber, phoneNumberPattern)) return true;
            DisplayAlert("Validation Error", "Invalid Phone Number format. Please enter a 10-digit phone number.", "OK");
            return false;
        ...
        private bool ValidateMembershipType(string membershipType) {
            ...
            var inputMembershipType = membershipType.ToLower();
        ...
        private bool ValidateTrainingSessionsPerWeek(string trainingSessionsPerWeekText) {
            ...
            if (!int.TryParse(trainingSessionsPerWeekText, out var trainingSessionsPerWeek)) {
        ...
        private bool ValidateBooleanField(Entry entry, string fieldName) {
            ...
            if (!bool.TryParse(entry.Text.ToLower(), out var fieldValue)) {
        ...
        private bool ValidatePostalCode(string postalCode) {
            ...
            var postalCodePattern = @"^[A-Za-z]\d[A-Za-z] \d[A-Za-z]\d$|^[A-Za-z]\d[A-Za-z]\d[A-Za-z]\d$";
